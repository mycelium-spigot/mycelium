...
public enum EnumProtocol {
    ...
    public Packet a(EnumProtocolDirection enumprotocoldirection, int i) throws IllegalAccessException, InstantiationException {
        Class<? extends Packet> oclass =  this.j.get(enumprotocoldirection).get(i);
	...
	for(EnumProtocol enumprotocol : values()) {
        ...
        for (EnumProtocolDirection enumprotocoldirection : enumprotocol.j.keySet()) {
            ...
            for (Iterator<Class<? extends Packet>> iterator1 = enumprotocol.j.get(enumprotocoldirection).values().iterator(); iterator1.hasNext(); EnumProtocol.h.put(oclass, enumprotocol)) {
                oclass = iterator1.next();
